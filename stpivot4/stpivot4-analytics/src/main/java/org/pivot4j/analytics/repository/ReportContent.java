...
public class ReportContent implements Serializable {
	...
	private Logger logger = LoggerFactory.getLogger(ReportContent.class);
	...
	public ReportContent(ViewState state) {
		...
		try {
			configuration.addProperty("custommembers", Encoder.encodeObject(state.getCustomMembers()));
		}
		catch(Throwable th) {
	...
	public ViewState read(ViewState state, DataSourceManager manager,
		...
		try {
			String encodedCustomMembers = configuration.getString("custommembers");
			List<CustomMember> cml = (List<CustomMember>) Encoder.decodeToObject(encodedCustomMembers);
			if (cml!=null) {
				state.setCustomMembers(cml );
			}
		}
		catch(Throwable th) {
			logger.warn(th.getMessage(), th);
