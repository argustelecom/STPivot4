...
var waitDialogDelayedTimedOut = null;

function showWaitDialog() {
	
		waitDialogDelayedTimedOut = setTimeout(
				function(){
					var waitDialog = PF("waitDialog");
					if (waitDialog) {
						waitDialog.block();
					}
					waitDialogDelayedTimedOut = null;
				}
				, 500
			);
}
...
function hideWaitDialog() {
	if (waitDialogDelayedTimedOut!=null) {
		clearTimeout(waitDialogDelayedTimedOut);
	}
